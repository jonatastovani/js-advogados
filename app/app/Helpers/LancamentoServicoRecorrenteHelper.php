...
class LancamentoServicoRecorrenteHelper
{
    ...
    public static function processarLancamentosServicosRecorrentesPorTenant(string $tenantId): void
    {
        ...
        foreach ($lancamentos as $agendamento) {
            try {
                self::processarAgendamento($agendamento->id);
            } catch (\Exception $e) {
                // Log do erro, mas continua com os outros lancamentos
                Log::error("Erro ao processar Lançamento de Serviço Recorrente ID {$agendamento->id}: {$e->getMessage()}");
            }
        }
    }
