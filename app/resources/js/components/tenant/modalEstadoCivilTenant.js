...
export class modalEstadoCivilTenant extends modalSearchAndFormRegistration {
    ...
    #objConfigs = {
        ...
        querys: {
            consultaFiltros: {
                ...
                url: window.apiRoutes.baseEstadoCivilTenant,
                urlSearch: `${window.apiRoutes.baseEstadoCivilTenant}/consulta-filtros`,
            }
        },
    };
    ...
    */
    ...
    constructor() {
        super({
            idModal: "#modalEstadoCivilTenant",
        });
    ...
    #addEventosPadrao() {
        ...
        const formDataSearchModalEstadoCivilTenant = modal.find('#formDataSearchModalEstadoCivilTenant');

        modal.find('.btn-new-register').on('click', async () => {
            self._updateTitleRegistration('Novo Estado Civil');
        });

        formDataSearchModalEstadoCivilTenant.find('.btnBuscar').on('click', async (e) => {
            e.preventDefault();
    ...
    #addEventosRegistrosConsulta(item) {
        ...
        $(`#${item.idTr}`).find(`.btn-delete`).click(async function () {
            const response = await self._delButtonAction(item.id, item.nome, {
                title: `Exclusão de Estado Civil`,
                message: `Confirma a exclusão do estado civil <b>${item.nome}</b>?`,
                success: `Estado Civil excluído com sucesso!`,
                button: this,
    ...
    #saveVerifications(data, formRegistration) {
        let blnSave = commonFunctions.verificationData(data.nome, { field: formRegistration.find('input[name="nome"]'), messageInvalid: 'O nome do Estado Civil deve ser informado.', setFocus: true });
        return blnSave;
    }
}